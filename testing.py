...
class PlotShower:
    ...
    def animate_construction(self):
        ...
        for p in range(self.places):
            ...
            if os.path.exists(self.data_file_path):
                ...
                n = (2 ** p) * self.number_of_deltas
                x_data = data['x_data'].tolist()[:int((2 ** p) * self.number_of_deltas - self.number_of_deltas)]
                y_data = data['y_data'].tolist()[:int((2 ** p) * self.number_of_deltas) - self.number_of_deltas]
                colors = data['colors'].tolist()[:int((2 ** p) * self.number_of_deltas) - self.number_of_deltas]
            else:
                # If the file does not exist
                print(f"ERROR2356432 No data file exists")
            
            # Save temporary figure as PNG
            temporary_image_file_path_writing = os.path.join(self.number_direction, self.angle_folder, self.base_folder, f"temporaryfigure{p}.png")
        ...
        animated_construction_file_path_writing = os.path.join(self.number_direction, self.angle_folder, self.base_folder, f"{self.number_direction}_animated_construction_of_(piDividedBy{self.denom})_and_{self.alpha.real}+{self.alpha.imag}i).gif")
        ...
        print(f"\n\n\n\n\n\n\n\nGIF saved as {animated_construction_file_path_writing}\n\n\n\n\n\n\n\n\n\n")
    ...
    def check_slider_changes(self):        
        ...
        if (self.previous_alpha != self.alpha or self.denom != self.previous_denom or self.max_term_limit != self.previous_max_term_limit or self.previous_point_size != self.point_size):
            self.previous_alpha = self.alpha
            self.previous_denom = self.denom
    ...
    def navigate_and_extract(self):
        ...
        self.limited_image_file_path_reading = os.path.join(self.number_direction, self.angle_folder, self.base_folder, f"figure{self.max_term_limit}.png")
        self.max_image_file_path = os.path.join(self.number_direction, self.angle_folder, self.base_folder, 'figureMax.png')
    ...
    def draw_point(self):
        ...
        #debugging#print(self.x_data, self.y_data)
    ...
    def save_limited_image(self):
        """Save the current plot as an image file."""
        self.limited_image_file_path_writing = os.path.join(self.number_direction, self.angle_folder, self.base_folder, f"figure{self.places}.png")
